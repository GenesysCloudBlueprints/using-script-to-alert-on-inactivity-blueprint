...
export const Channel = ({
	...
	useEffect(() => {
		...
		if (token) {
			...
			axios({
				...
				url: `https://api.${process.env.REACT_APP_ENVIRONMENT}/api/v2/notifications/channels?includechannels=${token}`,
				method: 'GET',
				headers: {
				...
				.then((response) => {
					if (response.data.entities[0]) {
						...
						axios({
							url: `https://api.${process.env.REACT_APP_ENVIRONMENT}/api/v2/notifications/channels/${channelID}/subscriptions`,
							method: 'GET',
							headers: {
								Authorization: 'Bearer ' + token,
							},
							...
							.then((response) => {
								if (response.data.entities) {
									//subscriptions on that channel already exist
									let subExists = true;
							...
							.catch((error) =>
								console.log('error on get subscriptions: ', error)
							);
					}
				})
			...
			if (!channelExists) {
				...
				axios({
					url: `https://api.${process.env.REACT_APP_ENVIRONMENT}/api/v2/notifications/channels`,
					method: 'POST',
					headers: {
						Authorization: 'Bearer ' + token,
					},
	...
	function listenToNotification(
		...
		socket.onmessage = (event) => {
			...
			if (eventData.eventBody.id === interactionID) {
				//if event is relevant to the conversation
	...
	function processEvent(eventBody: any) {
		...
		return customerState !== 'disconnected' && userState !== 'disconnected'; //check if both agent and client are active in chat
	}
	...
	return (
		<>
			...
			{chatState.current === 'disconnected' &&
				prevStatus.current === 'connected' && <PopupTimer />}
		</>
	);
};
